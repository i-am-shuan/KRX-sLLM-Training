...
def prepare_dataset(tokenizer):
    """데이터셋 준비"""
    try:
        logger.info("Loading Finance Alpaca dataset...")
        dataset = load_dataset("gbharti/finance-alpaca")
        
        logger.info(f"Dataset loaded. Train size: {len(dataset['train'])} examples")

        def format_conversation(instruction, input_text, output):
            """대화를 Qwen2 형식으로 포맷팅"""
            # input이 있는 경우와 없는 경우를 처리
            if input_text.strip():
        ...
        def preprocess_function(examples):
            """데이터셋 전처리"""
            formatted_texts = [
                format_conversation(instruction, input_text, output)
